...
Add the sources via CMake `add_subdirectory` or FetchContent.
...
```cpp
// Configure the generated client host to:
//   https://api.1msg.io/{MSG_INSTANCE_ID}
// Pass MSG_API_TOKEN as the request token query parameter.
// Use MessagingApi::sendMessage for text replies inside the 24h window.
```
...
First release is Git-based (`1msg/1msg-sdk-cpp`). Conan/vcpkg packaging may follow later.
