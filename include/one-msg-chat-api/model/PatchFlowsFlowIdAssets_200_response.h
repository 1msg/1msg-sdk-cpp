/**
 * 1MSG WhatsApp Business API (Public)
 * Public client API for sending messages, managing groups, flows, and templates.  This is the API exposed to end customers via platform.1msg.io  **Authentication:** All requests require `token` query parameter with JWT or API key.  **Documentation:** https://help.1msg.io/platform-1msg/getting-start/quick-start **API Docs:** https://docs.1msg.io/ 
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: support@1msg.io
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.22.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * PatchFlowsFlowIdAssets_200_response.h
 *
 * 
 */

#ifndef ONEMSG_CHAT_MODEL_PatchFlowsFlowIdAssets_200_response_H_
#define ONEMSG_CHAT_MODEL_PatchFlowsFlowIdAssets_200_response_H_

#include <boost/optional.hpp>

#include "one-msg-chat-api/ModelBase.h"

#include "one-msg-chat-api/AnyType.h"
#include <map>
#include <cpprest/json.h>
#include <vector>

namespace onemsg {
namespace chat {
namespace model {



class  PatchFlowsFlowIdAssets_200_response
    : public ModelBase
{
public:
    PatchFlowsFlowIdAssets_200_response();
    virtual ~PatchFlowsFlowIdAssets_200_response();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;


    /////////////////////////////////////////////
    /// PatchFlowsFlowIdAssets_200_response members


    bool isSuccess() const;
    bool successIsSet() const;
    void unsetSuccess();
    void setSuccess(bool value);

    std::vector<std::map<utility::string_t, std::shared_ptr<AnyType>>> getValidationErrors() const;
    bool validationErrorsIsSet() const;
    void unsetValidationErrors();
    void setValidationErrors(const std::vector<std::map<utility::string_t, std::shared_ptr<AnyType>>>& value);

    /// <summary>
    /// Error details returned by the provider. May be a string or a JSON object.
    /// </summary>
    std::shared_ptr<AnyType> getError() const;
    bool errorIsSet() const;
    void unsetError();
    void setError(const std::shared_ptr<AnyType>& value);


    /// <summary>
    /// Get additional properties (properties not defined in the schema)
    /// </summary>
    std::map<utility::string_t, web::json::value> getAdditionalProperties() const;
    bool additionalPropertiesIsSet() const;
    void unsetAdditionalProperties();
    /// <summary>
    /// Set additional properties
    /// </summary>
    void setAdditionalProperties(const std::map<utility::string_t, web::json::value>& value);
    /// <summary>
    /// Add a single additional property
    /// </summary>
    void addAdditionalProperty(const utility::string_t& key, const web::json::value& value);

protected:
    bool m_Success;
    bool m_SuccessIsSet;

    std::vector<std::map<utility::string_t, std::shared_ptr<AnyType>>> m_ValidationErrors;
    bool m_ValidationErrorsIsSet;

    boost::optional<std::shared_ptr<AnyType>> m_Error;

    std::map<utility::string_t, web::json::value> m_AdditionalProperties;
    bool m_AdditionalPropertiesIsSet;
};


}
}
}

#endif /* ONEMSG_CHAT_MODEL_PatchFlowsFlowIdAssets_200_response_H_ */
